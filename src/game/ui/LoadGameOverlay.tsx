...
function slotSublabel(index: number): string {
  if (index === 0) return "Последняя запись из F5 или «Сохранить» в меню.";
  return "Ручной снимок — см. «Сохранить в слот…» в меню «?».";
}
...
export default function LoadGameOverlay({
  open,
  onClose,
  onAfterLoadSuccess,
}: {
  ...
  const onLoad = useCallback(
    async (index: number) => {
      const slot = useSaveSlotsStore.getState().slots[index];
      if (!slot) return;
      if (
        !window.confirm(
          "Загрузить этот слот? Несохранённый прогресс текущей сессии будет потерян."
        )
      ) {
        return;
      }
      ...
      if (!r.ok) {
        window.dispatchEvent(
          new CustomEvent("last-summon-toast", {
            detail: {
              message: r.error ?? "Не удалось загрузить слот.",
            },
          })
        );
        return;
      ...
      window.dispatchEvent(
        new CustomEvent("last-summon-toast", {
          detail: { message: `Загружено: ${slotHeadline(index)}` },
        })
      );
      onClose();
      onAfterLoadSuccess();
    },
    [onAfterLoadSuccess, onClose]
  );
  ...
  return (
    <PaperModalChrome title="Загрузка игры" onClose={onClose} fitContent>
      <div className="flex max-h-[min(78vh,560px)] min-h-0 flex-col gap-4 overflow-y-auto px-0.5 py-1 sm:px-1 sm:py-2">
        <div className="flex gap-2.5 rounded-lg border border-[#c9b89a]/60 bg-[#faf6ee]/90 px-3 py-2.5 shadow-sm">
          ...
          <div className="min-w-0 text-[11px] leading-snug text-[#4a4034] sm:text-xs">
            <span className="font-semibold text-[#2e261c]">Загрузка</span>
            <p className="mt-1 text-[#5c4d38]">
              Слот <span className="font-mono font-semibold">0</span> — то же,
              что обновляется при{" "}
              <span className="font-semibold">F5</span>. Чтобы записать игру в
              слоты <span className="font-mono font-semibold">1–4</span>, откройте
              меню «?» → «Сохранить в слот…».
            </p>
          </div>
        </div>
              ...
              <li key={index} role="listitem">
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {filled ? (
                      <p className="mt-0.5 text-[11px] leading-snug text-[#5c4d38] sm:text-xs">
                        {slotSublabel(index)}
                      </p>
                    ) : null}
